...
std::vector<std::vector<std::string>> tabelaIdxHash(int B, std::vector<std::string> padroes, int nP, int c, int M){

 	std::vector<std::vector<std::string>> vecpermu(nP,std::vector<std::string>());
	...
	std::vector<std::vector<std::string>> tabelaHash(vpSize,std::vector<std::string>());
	tabelaHash  = TabelaHash(vecpermu,c,M);
...
long long int Hash(std::string strBloco, int c, int M){
    
    long long int hash = 0;
    for (int i = 0; i <strBloco.length() ; ++i) {
        hash = hash + strBloco[i] * pow(c,i);
	
       // hash = hash + strBloco[i] * pow(2,i);
    }
