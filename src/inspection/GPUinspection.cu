...
void GPUInspection::exec(std::shared_ptr<Packet> pkt)
{
    ...
    if(sizeBuffer_ >= limitBuffer_)
    {
        ...
        {
            l_sizePacket[i] = buffer_[i].get()->size_payload_;
            l_beginPacket[i] = index;
            strcpy(l_packet + index,reinterpret_cast<const char*>(buffer_[i].get()->payload_));
...
int GPUInspection::getIndex(pthread_t pid)
{
